...
class PuzzleLogic {
    ...
    //|Public|
    public Reset(state: PuzzleLogicState, log = false, seed = null): void {
    ...
    public Tick(state: PuzzleLogicState): void {
    ...
    public RequestMoveSelector(state: PuzzleLogicState, row: number, col: number): void {
    ...
    public RequestSwitch(state: PuzzleLogicState): void {
        if (!state.Active.Swap) {
    ...
    public UpdateActive(state: PuzzleLogicState): void {
    ...
    public SaveState(state: PuzzleLogicState): object {
    ...
    public LoadState(state: PuzzleLogicState, loadState): void {
    ...
    //Ticks
    private HoverTick(state: PuzzleLogicState): void {
    ...
    private FallTick(state: PuzzleLogicState): void {
    ...
    private MoveBlocksUpTick(state: PuzzleLogicState): void {
    ...
    private SwapBlocksTick(state: PuzzleLogicState): void {
        state.Ticks.Swap++;
    ...
    private RemoveBlocksTick(state: PuzzleLogicState): void {
    ...
    //Checks
    private CheckForSets(state: PuzzleLogicState, chain: number): void {
        chain++;
    ...
    private CheckForHover(state: PuzzleLogicState, chain: number, switchedBlocks: boolean): void {
    ...
    private CheckForFalling(state: PuzzleLogicState): void {
        var chain: number = 0;
        for (var row: number = 1; row < Constants.MAX_ROWS; row++) {
            for (var col: number = 0; col < Constants.MAX_COLS; col++) {
                if (state.Blocks[row][col].State == BlockState.Falling) {
    ...
    //Other
    private CreateSetupBlocks(state: PuzzleLogicState): void {
        for (var i: number = 0; i < Constants.STARTING_ROWS; i++) {
            this.AddBlockRow(state, i);
        }
    }
    private RowChange(state: PuzzleLogicState): void {
    ...
    private MoveBlocksUpOneRow(state: PuzzleLogicState): void {
    ...
    private AddBlockRow(state: PuzzleLogicState, row: number): void {
    ...
    //Condtional
    private CheckForGameOver(state: PuzzleLogicState): void {
        for (var col: number = 0; col < Constants.MAX_COLS; col++) {
    ...
    private IsNewBlockVaild(state: PuzzleLogicState, blockRow: number, blockCol: number, blockColor: BlockColor): boolean {
    ...
    //Score
    private GetScore(state: PuzzleLogicState, chain: number): void {
        this.ChainScore(state, chain);
        for (var i: number = 0; i < state.SetCount; i++) {
            this.TotalBlockScore(state, state.BlockSets[i]);
        }
    }
    private ChainScore(state: PuzzleLogicState,chain: number): void {
    ...
    private TotalBlockScore(state: PuzzleLogicState,totalBlocks: number): void {
