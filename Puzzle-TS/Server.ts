import {
    ...
    PlayerJoinedRoomData, PlayerLeftRoomData, UpdatePlayerNameData, SendMessageData, UpdateLogData, StartData, RoomClosedData, GameEnded, NetworkState, PuzzleLogicState
} from './dataTypes.js'
...
class Server {
    ...
    private PuzzleLogService: PuzzleLogService;
    ...
    private MergeLog(state: Room, player: Player, logItems: LogItem[]) {
        ...
        //  //  this.PuzzleLoader.AdvanceToEndOfLog(this.PuzzleLogic, player.LogicState, player.PuzzleLogState, serverLogId + 1);
        //}
        //else {
        //    desync = true;
        //    for (let j = 0; j < currentTick - serverTick; j++) {
        //        this.PuzzleLogic.Tick(player.LogicState);
        //    }
        //}
    ...
    private StartGame(state: Room) {
        ...
        for (let i = 0; i < state.Players.length; i++) {
            this.PuzzleLogic.Reset(state.Players[i].LogicState, true)
        } 
    ...
    private Join(player: Player, state: Room = null) {
        ...
        if (state.Players.length < 2) {
            ...
            this.PuzzleLogic.Reset(playerState.LogicState, true, this.Random());
    ...
    private LogNetworkCallIn(action: string, playerKey: string, data: object) {
        this.NetworkCallsIn.push({ action: action, playerId: playerKey, data: data });
    }
}
