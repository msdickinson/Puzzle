...
class Network {
    ...
    public JoinGame(socket: WebSocket, name: string| null = null) {
        let data = new JoinGameRequest();
        data.Name = name;
        socket.send(this.ParseRequest("JoinGame", data));
    }
    public LeaveGame(socket: WebSocket, roomId: string| null = null) {
        let data = new LeaveGameRequest();
        data.RoomId = roomId;
        socket.send(this.ParseRequest("LeaveGame", data));
    }
    public JoinRoom(socket: WebSocket, roomId: string| null = null) {
        let data = new JoinRoomRequest();
        data.RoomId = roomId;
        socket.send(this.ParseRequest("JoinRoom", data));
    }
    public LeaveRoom(socket: WebSocket, roomId: string| null = null) {
    ...
    public UpdateName(socket: WebSocket, name: string, roomId: string| null = null) {
    ...
    public SendMessage(socket: WebSocket, message: string, roomId: string| null = null) {
    ...
    public SendLog(socket: WebSocket, logItems: LogItem[], roomId: string | null = null) {
